...
/*
 * Creates a sandboxed Twig environment for template_from_string rendering.
 * This prevents SSTI (Server-Side Template Injection) when user-supplied
 * content is compiled as Twig code (e.g., email templates, payment forms).
 */
$di['twig_sandbox'] = $di->protect(function (Twig\Environment $twig) use ($di) {
    $tags = ['if', 'for', 'set', 'verbatim'];
    $filters = ['escape', 'e', 'raw', 'upper', 'lower', 'capitalize', 'title', 'nl2br',
        'trans', 'link', 'alink', 'autolink', 'markdown', 'truncate', 'timeago',
        'daysleft', 'size', 'number', 'money', 'money_without_currency',
        'money_convert', 'money_convert_without_currency', 'gravatar',
        'period_title', 'asset_url', 'library_url', 'mod_asset_url',
        'img_tag', 'script_tag', 'stylesheet_tag'];
    $methods = [];
    $properties = [];
    $functions = ['range', 'cycle', 'constant', 'max', 'min', 'date', 'include'];

    $policy = new Twig\Sandbox\SecurityPolicy($tags, $filters, $methods, $properties, $functions);
    $sandbox = new Twig\Extension\SandboxExtension($policy, true);

    $sandboxedTwig = clone $twig;
    $sandboxedTwig->addExtension($sandbox);

    return $sandboxedTwig;
});
