...
final class FOSSBilling_Installer
{
    ...
    private function connectDatabase(): void
    {
        ...
        // Sanitize the database name (allow only alphanumeric and underscore)
        $dbName = preg_replace('/[^a-zA-Z0-9_]/', '', $this->session->get('database_name'));

        // Attempt to create the database.
        try {
            $this->pdo->exec('CREATE DATABASE `' . $dbName . '` CHARACTER SET utf8 COLLATE utf8_general_ci;');
        } catch (PDOException) {
    ...
    private function install(): bool
    {
        ...
        if (!$this->filesystem->exists(PATH_HTACCESS)) {
            try {
                ...
                if (!str_starts_with(trim($content), '# BEGIN FOSSBilling')) {
                    throw new Exception('Downloaded .htaccess file failed integrity check.');
                }
                $this->filesystem->dumpFile(PATH_HTACCESS, $content);
            } catch (Exception $e) {
