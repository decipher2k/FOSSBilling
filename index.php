 ...
 * Internally the return and cancel URLs get a restore_session GET parameter attached to them with a HMAC-signed session ID token to restore, so we do so here.
 */
if (!empty($_GET['restore_session']) && !empty($_GET['restore_token'])) {
    $restoreToken = $_GET['restore_token'];
    $restoreSessionId = $_GET['restore_session'];
    $expectedToken = hash_hmac('sha256', $restoreSessionId, $config['info']['salt']);
    if (hash_equals($expectedToken, $restoreToken)) {
