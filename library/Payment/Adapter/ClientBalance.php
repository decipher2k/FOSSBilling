...
class Payment_Adapter_ClientBalance implements FOSSBilling\InjectionAwareInterface
{
    ...
    {
        /*
         * ClientBalance is a server-side payment method. Verify:
         * 1. The invoice ID is valid
         * 2. The gateway matches this adapter
         * 3. The invoice is unpaid (no double-spend)
         */
        $invoice_id = $data['get']['invoice_id'] ?? $data['post']['invoice_id'] ?? null;
        $gateway_id = $data['get']['gateway_id'] ?? $data['post']['gateway_id'] ?? null;
        ...
        if (!$gatewayModel || $gatewayModel->id != $gateway_id) {
            return false;
        }

        $invoiceModel = $this->di['db']->load('Invoice', $invoice_id);
        if (!$invoiceModel || $invoiceModel->status !== 'unpaid') {
            return false;
        }
